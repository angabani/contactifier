...
export function contactLabel(label: string | undefined, fallback: string): string {
  const trimmed = label?.trim();
  if (!trimmed) return fallback;
  return trimmed.match(/^_\$!<(.+)>!\$_$/)?.[1]?.trim() || trimmed;
}
...
export function contactReviewValues(contact: CanonicalContact): readonly ContactReviewValue[] {
  return Object.freeze([
    ...contact.phoneNumbers.map((item) => ({
      id: `phone:${item.id}`,
      kind: 'phone' as const,
      label: contactLabel(item.label, 'Phone'),
      value: item.value.raw,
    })),
    ...contact.emailAddresses.map((item) => ({
      id: `email:${item.id}`,
      kind: 'email' as const,
      label: contactLabel(item.label, 'Email'),
      value: item.value,
    })),
  ]);
}
